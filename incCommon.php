	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = [
			...
			'alumnos' => ['Alumnos', '', 'resources/table_icons/client_account_template.png', 'None'],
			'movimientos' => ['Movimientos', '', 'resources/table_icons/table_money.png', 'None'],
		];
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'alumnos' => "`alumnos`.`id` as 'id', `alumnos`.`carnet` as 'carnet', `alumnos`.`nombre_completo` as 'nombre_completo', `alumnos`.`facultad` as 'facultad', `alumnos`.`turno` as 'turno', DATE_FORMAT(`alumnos`.`fecha_matri`, '%e/%c/%Y') as 'fecha_matri', `alumnos`.`numero` as 'numero'",
			'movimientos' => "`movimientos`.`id` as 'id', IF(    CHAR_LENGTH(`alumnos1`.`carnet`), CONCAT_WS('',   `alumnos1`.`carnet`), '') as 'carnet', `movimientos`.`descripcion` as 'descripcion', `movimientos`.`comentarios` as 'comentarios', DATE_FORMAT(`movimientos`.`fecha_pago`, '%e/%c/%Y %l:%i%p') as 'fecha_pago', `movimientos`.`importe_nio` as 'importe_nio', `movimientos`.`tipo_pago` as 'tipo_pago'",
		];
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'alumnos' => "`alumnos` ",
			'movimientos' => "`movimientos` LEFT JOIN `alumnos` as alumnos1 ON `alumnos1`.`id`=`movimientos`.`carnet` ",
		];
	...
	function htmlUserBar() {
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Sistema de Control de Pagos</a>
			</div>
