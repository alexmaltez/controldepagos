		...
		$all_tables = array(
			/* ['table_name' => [table props assoc array] */   
				...
				'movimientos' => array(
					'Caption' => 'Movimientos',
					'Description' => '',
					'tableIcon' => 'resources/table_icons/table_money.png',
					'group' => $tg[0],
					'homepageShowCount' => 1
				),
		);
			...
			*/
			...
			if(!$result = @db_query($statment, $db_link)) {
				if(!stristr($statment, "show columns")) {
					...
					if(getLoggedAdmin()) $errorMsg .= "<pre class=\"ltr\">{$Translation['query:']}\n" . htmlspecialchars($statment) . "</pre><p><i class=\"text-right\">{$Translation['admin-only info']}</i></p>";
		...
		if($schema === null) {
			$schema = [
				'alumnos' => [
					...
					'carnet' => ['appgini' => "INT(6) NOT NULL UNIQUE"],
					'nombre_completo' => ['appgini' => "VARCHAR(40) NOT NULL"],
					'facultad' => ['appgini' => "VARCHAR(40) NOT NULL"],
					'turno' => ['appgini' => "VARCHAR(40) NOT NULL"],
					'fecha_matri' => ['appgini' => "DATE NOT NULL"],
					'numero' => ['appgini' => "INT(8) NOT NULL"],
				],
				'movimientos' => [
					'id' => ['appgini' => "INT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT"],
					'carnet' => ['appgini' => "INT UNSIGNED NOT NULL"],
					'descripcion' => ['appgini' => "VARCHAR(40) NOT NULL"],
					'comentarios' => ['appgini' => "VARCHAR(40) NULL"],
					'fecha_pago' => ['appgini' => "DATE NOT NULL"],
					'importe_nio' => ['appgini' => "DOUBLE(10,2) NOT NULL"],
					'tipo_pago' => ['appgini' => "VARCHAR(40) NOT NULL"],
				],
			];
		}
	...
	function lookupQuery($tn, $lookupField) {
		...
		*/
		$lookupQuery = [
			...
			'movimientos' => [
				'carnet' => 'SELECT `alumnos`.`id`, `alumnos`.`carnet` FROM `alumnos` ORDER BY 2',
			],
		];
